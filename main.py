...
async def double_or_half(interaction: discord.Interaction):
    ...
    if coins > 0:
        ...
        if success:
            await interaction.followup.send(embed=discord.Embed(title=f"{interaction.user} tries their hand at resolving their debt...", description=f"Your debt has been halved! New balance: {change_player_coin(id, name, int(coins / 2), True)}<:coin1:910247623787700264>", color=0x00FF00))
        else:
            await interaction.followup.send(embed=discord.Embed(title=f"{interaction.user} tries their hand at resolving their debt...", description=f"Lol. Your debt has been doubled. New Balance: {change_player_coin(id, name, int(coins * 2), True)}<:coin1:910247623787700264>", color=0xFF0000))
